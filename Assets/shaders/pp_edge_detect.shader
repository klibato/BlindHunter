...
#include "common/classes/Depth.hlsl"
    ...
    float g_WaveSpeedFactor < Attribute("g_WaveSpeedFactor");
    Default(3.0);
    > ;
    float g_WaveThickness < Attribute("g_WaveThickness");
    Default(80.0);
    > ;
    ...
    float IsInsideWaveFront(float3 worldPos, float3 sourcePos, float intensity, float age, float lifetime)
    {
        ...
        // Atténuation EXPONENTIELLE avec la distance (bug "voir à travers les murs")
        // L'onde meurt vite avec la distance, plus aucun effet au-delà d'une certaine portée
        float maxRange = intensity * 2.5; // portée max basée sur l'intensité du bruit
    ...
    float4 MainPs(PixelInput i) : SV_Target0
    {
        ...
        float sobelX = -1.0 * tl + 1.0 * tr + -2.0 * l + 2.0 * r + -1.0 * bl + 1.0 * br;
        float sobelY = -1.0 * tl + -2.0 * t + -1.0 * tr + 1.0 * bl + 2.0 * b + 1.0 * br;
